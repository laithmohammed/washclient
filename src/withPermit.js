...
export default function withPermit(ComponentToProtect) {
  return class extends Component {
    ...
    componentDidMount() {
      ...
      // this action is same as './withoutLog' but with reverse meaning
      fetch(`${Params.originServer}/checkPermit/${Fun.getCookie('X-auth-token')}`)
        .then(res => res.json()).then(data=>{
          console.log(data)
          if (data.authorized === "valid token") { this.setState({ loading: false, redirect: false }); }
    ...
    render() {
      ...
      if (!loading) {
        if (redirect) { view = <Redirect to="/login" /> }
